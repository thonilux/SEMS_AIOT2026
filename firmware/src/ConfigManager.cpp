...
ModbusConfig ConfigManager::loadModbusConfig() {
  ...
  cfg.pm_count = prefs.getUChar("pm_count", 3);
  if (cfg.pm_count < 1 || cfg.pm_count > 3) cfg.pm_count = 3;
  ...
  cfg.preset[0] = prefs.getUChar("preset0", 1);
  cfg.preset[1] = prefs.getUChar("preset1", 1);
  cfg.preset[2] = prefs.getUChar("preset2", 1);
...
bool ConfigManager::saveModbusConfig(const ModbusConfig& cfg) {
  ...
  ok &= prefs.putUChar("pm_count", cfg.pm_count) > 0;
  ...
  ok &= prefs.putUChar("preset0", cfg.preset[0]) > 0;
  ok &= prefs.putUChar("preset1", cfg.preset[1]) > 0;
  ok &= prefs.putUChar("preset2", cfg.preset[2]) > 0;
