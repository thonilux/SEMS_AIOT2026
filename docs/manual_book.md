# Buku Panduan Singkat — Alat Perekam Data Energi (Energy Data Gateway)
*Panduan mudah untuk memahami cara kerja dan pengoperasian alat perekam data energi SEMS-AIOT V1.2.*
...
2. **Terminal Kabel RS485:** Terletak di tengah bawah. Jalur komunikasi serial kabel untuk mengambil data dari perangkat meteran listrik (Modbus).
...
Layar kecil di bagian depan menampilkan status koneksi alat secara langsung menggunakan simbol gambar yang mudah dipahami:

![Layar Utama](./oled/page0-dashboard.png)

*   **Jam & Tanggal (Kuning - Atas):** Menampilkan waktu saat ini yang menyesuaikan otomatis lewat internet.
...
## 3. Cara Menggunakan Tombol Sentuh (Touch Sensor)

Untuk menjaga kerapatan kotak agar debu tidak masuk, alat ini menggunakan satu **tombol sentuh tersembunyi** di sisi kanan layar kecil untuk melakukan semua pengaturan. Tombol ini tidak terlihat menonjol dari luar kotak — cukup sentuh permukaan di sebelah kanan layar OLED. Setiap kali disentuh, akan menyala **lampu indikator kecil berwarna merah** sebagai tanda sentuhan terdeteksi oleh alat.

*   **Sentuh Sekali (Ketuk/Tap):** Untuk menggeser pilihan menu ke bawah.
*   **Sentuh dan Tahan 3 Detik (Hold):** Untuk masuk ke **Menu Pengaturan**.
*   **Sentuh dan Tahan 2 Detik (Hold):** Untuk memilih menu yang ditunjuk / kembali ke layar utama.

---

## 4. Menu Pengaturan (Settings Menu)
...
1.  **Boot Mode `[NRM / CFG]` (Mode Nyala Alat)**
    *   **Normal `[NRM]`:** Mode kerja biasa untuk membaca data dan langsung mengirimkannya ke server pusat.
    *   **Config `[CFG]`:** Mode setting. Alat akan memancarkan WiFi sendiri (`SEMS-SETUP`) agar Anda bisa tersambung lewat HP untuk mengatur nama WiFi atau alamat server.
...
2.  **View Info (Lihat Info Detail)**
    *   Digunakan untuk melihat detail alamat IP internet, nama WiFi, dan status alat secara mendalam melalui 4 halaman info:
...
Jika alat baru pertama kali dipasang, atau Anda ingin mengganti password WiFi dan zona waktu, ikuti langkah mudah berikut:
...
4. Alat akan mati sejenak dan menyala ulang (restart). Layar depan akan menampilkan tulisan **Config Mode**.
...
*   **Mengatur Internet WiFi & Timezone (Waktu):** 
    Pilih menu **Network & WiFi** di web browser.
...
*   **Mengatur Modbus (Meteran RS485):**
    Pilih menu **Modbus** untuk menyesuaikan alamat meteran listrik (Slave ID), kecepatan data (Baud Rate), dan tipe meteran yang dipasang.
...
| **Komunikasi Data** | Protokol RS485 Modbus RTU (Terminal Blok 3-pin) |
| **Sensor Tombol** | Tombol sentuh praktis (tanpa perlu ditekan keras) |
| **Layar Informasi** | Layar kecil 2 warna (Kuning di atas, Biru di bawah) |
